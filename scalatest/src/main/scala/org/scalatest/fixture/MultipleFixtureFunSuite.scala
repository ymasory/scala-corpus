 ...
 * a <code>FixtureFunSuite</code>, different tests may take different types of fixtures. This trait extends <code>FixtureFunSuite</code>
 ...
 * 
 * <p>
 * Subclasses of this trait must, therefore, do two things differently from a plain old <code>org.scalatest.FunSuite</code>:
 * </p>
 * 
 * <ol>
 ...
 * 
 * class MySuite extends MultipleFixtureFunSuite {
 * 
 ...
 *   test("takes a string fixture") { (s: String) =>
 *     assert(s === "howdy")
 *   }
 * 
 *   // A test that takes a List[Int] fixture
 *   test("takes a list fixture") { (list: List[Int]) =>
 *     assert(list.size === 1)
 *   }
 * 
 ...
 * reason this works is that <code>MultipleFixtureFunSuite</code> inherits an implicit conversion from a by-name parameter to
 * <code>FixtureParam => Unit</code> from supertrait <code>FixtureFunSuite</code>. This implicit conversion is used to enable 
 ...
 * 
 * <p>
 * Note that in a <code>FixtureFunSuite</code>, you need to specify the type of the fixture explicitly so the compiler knows
 ...
 * test("takes a list fixture") { list =>
 *   assert(list.size === 1)
 * }
 * </pre>
 *
 * @author Bill Venners
 */
trait MultipleFixtureFunSuite extends FixtureFunSuite with ConfigMapFixture
